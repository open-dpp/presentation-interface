  ...
  <Disclosure v-slot="{ open }" as="header" class="bg-white shadow">
    <div class="mx-auto max-w-7xl px-2 sm:px-4 lg:divide-y lg:divide-gray-200 lg:px-8">
      ...
      <nav aria-label="Global" class="hidden lg:flex lg:space-x-8 lg:py-2">
        <button v-for="item in navigation" :key="item.name" :aria-current="item.current ? 'page' : undefined"
                :class="[item.current ? 'bg-gray-100 text-gray-900' : 'text-gray-900 hover:bg-gray-50 hover:text-gray-900', 'inline-flex items-center rounded-md px-3 py-2 text-sm font-medium']"
                @click="item.select">{{ item.name }}
        </button>
      </nav>
    </div>
    ...
    <DisclosurePanel aria-label="Global" as="nav" class="lg:hidden">
      <div class="space-y-1 px-2 pb-3 pt-2">
        <DisclosureButton v-for="item in navigation" :key="item.name" :aria-current="item.current ? 'page' : undefined"
                          :class="[item.current ? 'bg-gray-100 text-gray-900' : 'text-gray-900 hover:bg-gray-50 hover:text-gray-900', 'block rounded-md px-3 py-2 text-base font-medium']"
                          as="a"
                          @click="item.select">{{ item.name }}
        </DisclosureButton>
      </div>
